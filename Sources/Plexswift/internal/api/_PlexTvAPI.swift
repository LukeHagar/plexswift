...
class _PlexTvAPI: PlexTvAPI {
    ...
    public func getPin(request: Operations.GetPinRequest, server: PlexTvServers.GetPin?) async throws -> Response<Operations.GetPinResponse> {
        return try await client.makeRequest(
            with: try server?.server() ?? PlexTvServers.GetPin.default(),
    ...
    public func getToken(request: Operations.GetTokenRequest, server: PlexTvServers.GetToken?) async throws -> Response<Operations.GetTokenResponse> {
        return try await client.makeRequest(
            with: try server?.server() ?? PlexTvServers.GetToken.default(),
            configureRequest: { configuration in
...
private func handleGetPinResponse(response: Client.APIResponse) throws -> Operations.GetPinResponse {
    ...
    if httpResponse.statusCode == 200 { 
        ...
        if httpResponse.contentType.matchContentType(pattern: "application/json"), let data = response.data {
            do {
                return .fourHundredApplicationJsonObject(try JSONDecoder().decode(Operations.GetPinPlexTvResponseBody.self, from: data))
            } catch {
                throw ResponseHandlerError.failedToDecodeJSON(error)
            }
        }
